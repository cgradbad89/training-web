...
15. **Shared plan editor + real dirty-state (workout plan)**: The single-week-paginated editing UI (week pager, 7-day grid, inline row editor, drag-to-move/swap, copy-day, copy-week) is now a reusable component `src/components/PlanEditor.tsx`, driven by an injected `PlanEditorConfig<TEntry>` (type-specific `renderEntryRow` / `renderEntryEditor` / `makeNewEntry` / `weekSummaryLabel` / `copyEntryToDay` / `isRest`). It owns pagination + editing UI state but **not** persistence — every mutation calls `onUpdateWeek(weekIndex, entries)` and the caller writes (autosave-per-mutation preserved; still one doc `users/{uid}/plans/{planId}`, no subcollections). Pure orchestration helpers live in `src/utils/planEditorLogic.ts`. **`CrossTrainingPlanDetail` (workout plans) is wired to it; the running plan and its `/plans/[id]/edit` route are unchanged (separate prompt).** Dirty-state was corrected: `useUnsavedChanges` is now passed a **real `hasUnsavedChanges` flag** (true only while a mutation's autosave write is in flight, cleared in a `finally`), **not** the edit-mode boolean. Entering edit mode, or opening a row editor and clicking Cancel, no longer arms the browser "leave site?" warning. One consequence: a brief dirty window now also exists for view-mode writes (mark-complete / notes), where previously the guard was inert in view mode. The running edit page still passes `isEditing` to `useUnsavedChanges` and retains the old behavior until its migration.
