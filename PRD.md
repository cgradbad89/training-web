...
| Plan completion lifecycle | Medium | In Progress | Session A: `status` field + dual-write + read-time derive. Session B: Complete/Reopen controls on both headers, `completedAt` write/clear via `setPlanCompletion`, left-bar status sub-grouping (`groupPlansByStatus`). Remaining (Session C): completion summary view reusing Plan Insights metrics over the completed plan's full span. |
