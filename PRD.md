...
| Plans | `/(app)/plans` | Done | Running + workout plan management; week calendar; auto-match completion tracking; Goals tab (custom date-range distance/time/count goals with progress rings). Both plan types now edit IN-PLACE via the shared `PlanEditor` (single-week pagination): workout = `CrossTrainingPlanDetail`, running = `RunningPlanDetail`. The running header's "Edit Plan" toggles in-place edit (no longer routes away). |
| Plan Editor (legacy) | `/(app)/plans/[id]/edit` | Redirect | Standalone running-plan edit route — now a thin client redirect to `/plans` (running edits happen in-place). Route file/dir + old inline editor (WeekAccordion/EntryForm/"Save All") pending removal in a follow-up. |
