...
| Plans | `/(app)/plans` | Done | Running + workout plan management; week calendar; auto-match completion tracking; Goals tab (custom date-range distance/time/count goals with progress rings). Both plan types edit IN-PLACE via the shared `PlanEditor` (single-week pagination): workout = `CrossTrainingPlanDetail`, running = `RunningPlanDetail`. Symmetric headers (Set Active · Edit · Copy plan · trash); a single "Edit" toggle (in edit mode the plan name is inline-editable). The standalone `/plans/[id]/edit` route was removed. |
