...
// 6 runs spanning ~6 distinct weeks (all scored) → >= 3 weeks with data.
function populatedRuns(distanceMiles = 6): EfficiencyWorkout[] {
  return [3, 10, 17, 24, 31, 38].map((n) =>
    makeWorkout({ distanceMiles, secPerMile: 540, startDate: daysAgo(n) })
  );
}
