...
- **Test**: Run `npm test` after a passing build (`vitest run`, 1,202 baseline tests across 114 files — 1,191 passed, 11 skipped; as of the 2026-08-20 override-aware-plan-matching / AutoMatchRunner session). This number drifts — always trust a fresh `vitest run` over this doc, and correct this line when it does. Also watch for stray `.claude/worktrees/*` checkouts inflating the count (vitest's exclude only covers `node_modules`/`.git`); run `git worktree list` if the total looks off.
...
src/
  ...
  utils/            # Domain logic: metrics, pace, dates, trainingLoad, riegelFit, etc.
    __tests__/      # Vitest unit tests (1,161 tests across 108 files)
```
